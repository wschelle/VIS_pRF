pro ro1prf_3TMBr
  ...
  if file_test(rdir+subs[ii]+'/'+ses+'/func/'+tasks[0]+'/',/directory) then begin
    ...
    nifti1=tdir1+'NORDIC_'+task1+'-rwm.nii'
    tdir2=sdir+'func/'+task2+'/'
    nifti2=tdir2+'NORDIC_'+task2+'-rwm.nii'
    ...
    ;make simple mask
    mask1=intarr(nvox)
    mask1[where(mean(sm1,dimension=2) ge 400)]=1

    mask2=intarr(nvox)
    mask2[where(mean(sm2,dimension=2) ge 400)]=1
    ...
    grp=rp*0
    for i=0,11 do grp[i,*]=gauss_smooth(reform(rp[i,*]),1.175,/edge_mirror)
    rp=0
    ...
    smg=sm*0
    for i=0L,nvox-1 do if mask[i] eq 1 then smg[i,*]=gauss_smooth(reform(sm[i,*]),1.175,/edge_mirror)
    sm=0
    ;smg=sm
    ...
    smg=float(smg)
    print,tc,n_elements(tmat gt tc)/total(mask)
    ...
    fval=fltarr(nvox)
    for i=0L,nvox-1 do if zfitz[i,-1] gt 0 then fval[i]=(total((mean(reform(smg[i,*]))-yfitz[i,*])^2.)/(npar-1))/(zfitz[i,-1]/((nscans)-npar))
    r2val=fltarr(nvox)
